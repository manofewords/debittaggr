...
class DebitTaggr
{
	...
	{
		// make sure the session.save_path in php.ini is correct!
	...
	{
		if (!$_SESSION["IsAdmin"]) return; // not allowed
		
		//if($debit_amount*100 != array_sum($split_amount)*100) return; // quit, sum of splits is wrong
...
/*	
	function fullTextSearch($keywords)
	{
		$ids = array();
		...
		if($_SESSION["IsAdmin"])
		{
			...
			$query  = "SELECT id ";
			$query .= "FROM debits ";
			$query .= "WHERE MATCH(description) AGAINST ('".$keywords."') ";
			$query .= "OR MATCH(notes) AGAINST ('".$keywords."') ";
			$query .= "OR MATCH(tags) AGAINST ('".$keywords."') ";
			$result = @mysql_query($query,$this->db);
			while($row = mysql_fetch_array($result, MYSQL_ASSOC))
			{
				$ids[] = $row['id'];
